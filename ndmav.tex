...
\begin{definition}[Sum-Product Network]
  A \emph{Sum-Product Network} (SPN) $S$ over RVs $\mathbf X$ is a
  rooted weighted DAG (network) consisting of
  distribution \emph{leaves} (network inputs),  \emph{sum} and \emph{product}
  nodes (inner nodes).
  Let $\mathsf{ch}(n)$ be the set of all the child (input) nodes of
  a node $n$ and $\mathsf{pa}(n)$ the set of all its parent (output) nodes.
  % Each node $n$ has associated a \emph{scope},
  % $\mathsf{sc}(n)\subseteq\mathbf{X}$.
  % For a leaf $n$, $\mathsf{sc}(n)$ is the set of r.v. over which a
  % tractable, possibly unnormalized, distribution $\phi_{n}$ is defined.
  A leaf $n$ defines a tractable, possibly unnormalized, distribution
  $\phi_{n}$ over some RVs in $\mathbf X$.
  %The scope of an inner node $n$ is defined as $\mathsf{sc}(n)=\bigcup_{c\in\mathsf{ch}(n)}\mathsf{sc}(c)$.
  A nonnegative weight $w_{nc}$ is associated to each edge linking a sum node
  $n$ to $c\in\mathsf{ch}(n)$.
  We indicate with $\mathbf{w}$ the set of all
  weights in the network $S$ (network parameters).
  $S_{n}$ denotes the sub-network rooted at node $n$ and parametrized
  by $\mathbf{w}_{n}$, comprising all descendant nodes of $n$,
  recursively defined as the children of $n$ and their descendants.
\end{definition}
\end{frame}
...
\begin{definition}[Scope]
  Let $S$ be an SPN over RVs $\mathbf{X}$. 
  The \emph{scope} of a node $n$ in $S$ is denoted as $\mathsf{sc}(n)\subseteq\mathbf{X}$.
  The scope of a leaf node $n$ is defined as the set of RVs over which
  $\phi_{n}$ is defined.
...
\begin{definition}[SPN evaluation]
  Let $S$ be an SPN over RVs $\mathbf{X}$ with parameters $\mathbf{w}$
  and $S_{n}$ be the sub-network rooted at node $n$.
  ...
  Then, $S_{n}(\mathbf{x})$ can be computed as follows:
  \begin{equation}
    \label{eq:eval}
...
If a network $S$ is \emph{valid}, evaluating it corresponds to
evaluate a joint unnormalized probability distribution
$p_{\mathbf{X}}$.
That is, $\forall\mathbf{x},S(\mathbf{x})/Z=p(\mathbf{X = x})$, where $Z$ is the
normalizing \emph{partition} function defined as
$Z=\sum_{\mathbf{x}\sim \mathbf{X}}S(\mathbf{x})$.
%
This actually is possible since a complete and decomposable SPN
correctly compiles the
...
\begin{definition}[Completeness, decomposability and validity]
...
validity property holds~\cite{Poon2011}: the network can be evaluated
in a similar fashion as for the complete evidence case.
%
...
%
This mechanism highlights the recursive probabilistic interpretation
of SPNs, in that each sub-network modeling a probability distribution
over its own scope, shall output 1 as the probability of marginalizing
over all the RVs that scope.
%
As a consequence, even conditional probability queries are also computable
in tractable time, since $p(\mathbf{Q}|\mathbf{E}) = p(\mathbf{Q},
\mathbf{E})/p(\mathbf{E})$, for $\mathbf{Q}, \mathbf{E}\subset\mathbf{X}$.
%
In addition to that, setting all leaf outputs to 1 equals to compute the \emph{partition function}
$Z$ (whose value, for an unnormalized SPN, will differ from 1).
\end{frame}
...
Given an SPN $S$ over $\mathbf{X}$, to find an MPE assignment:
\begin{equation}
...
% \begin{equation}
%   \label{eq:mpe}
%   \mathbf{q}^{*}=\arg\max_{\mathbf{q}\sim \mathbf{Q}}p(\mathbf{E}, \mathbf{q})  
% \end{equation}
$S$ is transformed into a Max-Product Network (MPN) $M$.
%
$M$ is built by substituting each $n\in \mathbf S_{\oplus}$ for a max node computing
$\max_{c\in\mathsf{ch}(n)}w_{nc}M_{n}$ instead.
Then $M$ is evaluated bottom-up after setting all leaves $n$, $\mathsf{sc}(n)\subseteq
\mathbf{Q}$, to output 1.
A Viterbi-like top-down traversal traces back the MPE
assignment for each RV in $\mathbf{Q}$.
By starting from the root and following only the max output
...
\begin{frame}
  ...
  \begin{minipage}{0.4\linewidth}
    \begin{center}
      \only<1>{\includegraphics[width=.85\columnwidth]{figures/spn-eval-emb-I}}
      \only<2>{\includegraphics[width=.86\columnwidth]{figures/spn-eval-emb-II}}
    \end{center}
  \end{minipage}\hfill\begin{minipage}[t]{0.55\linewidth}
    ...
    (\colorbox{orange}{\textcolor{white}{sum}}, \colorbox{blue}{\textcolor{white}{product}} but not for
    \colorbox{purple}{\textcolor{white}{leaf}} nodes) activations
    \begin{align*}
    \mathbf{e}^{i}= \langle&\highlight[orange]{.55},\highlight[blue]{.66},\highlight[orange]{.55},\highlight[blue]{.66},\highlight[orange]{.55},\highlight[blue]{.66},\\
                   &\highlight[orange]{.55},\highlight[blue]{.66},\highlight[orange]{.55},\highlight[blue]{.66},\highlight[orange]{.55},\highlight[blue]{.66},\\
      &\highlight[orange]{.55},\highlight[blue]{.66}  \rangle
  \end{align*}}
...
\begin{frame}
  ...
  \end{minipage}\hfill\begin{minipage}[t]{0.55\linewidth}
    ...
    inner node activations filtered by node type
      \begin{align*}
        \mathbf{e}^{i}_{\mathsf{sum}}= \langle&\highlight[orange]{.55},\highlight[orange]{.66},\highlight[orange]{.55},\highlight[orange]{.66},\highlight[orange]{.55},\highlight[orange]{.66}\rangle\\
        \mathbf{e}^{i}_{\mathsf{prod}}=\langle  &\highlight[blue]{.55},\highlight[blue]{.66},\highlight[blue]{.55},\highlight[blue]{.66},\highlight[blue]{.55},\highlight[blue]{.66},\\
                               &\highlight[blue]{.55},\highlight[blue]{.66}  \rangle
      \end{align*}
...
\begin{frame}
  ...
  \begin{minipage}{0.4\linewidth}
    \begin{center}
      \only<1>{\includegraphics[width=.85\columnwidth]{figures/spn-eval-emb-III}}
      \only<2>{\includegraphics[width=.86\columnwidth]{figures/spn-eval-emb-IV}}
    \end{center}
  \end{minipage}\hfill\begin{minipage}[t]{0.55\linewidth}
    \vspace{-80pt}
    Build an embedding $\mathbf{e}^{i}\in\mathbb{R}^{d}$ for sample
    $$\mathbf{x}^{i}=\langle 0,1,0,1,1,1 \rangle$$
    \only<1>{by adding fictitious \textcolor{red}{sum} nodes over unique scopes}
    \only<2>{by adding fictitious sum nodes over
      unique scopes, then evaluating $S(\mathbf{x}^{i})$ and collecting they
      activations from  \colorbox{red}{\textcolor{white}{inner}}
      nodes,  and even \colorbox{purple}{\textcolor{white}{leaves}} 
      \begin{align*}
        \mathbf{e}^{i}_{\mathsf{w/o-leaves}}=
        \langle&\highlight[red]{.55},\highlight[red]{.66},\highlight[red]{.55},\highlight[red]{.66}\rangle \\
        \mathbf{e}^{i}_{\mathsf{w-leaves}}=
        \langle&\highlight[red]{.55},\highlight[red]{.66},\highlight[red]{.55},\highlight[red]{.66}\\
&\highlight[purple]{.55},\highlight[purple]{.66},\highlight[purple]{.55},\highlight[purple]{.66}\rangle \\
      \end{align*}}
  \end{minipage}
\end{frame}
