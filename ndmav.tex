...
\emph{complete} and \emph{decomposable}\customcite{Darwiche2009}\customcite{Poon2011}.
...
\onslide<1-8>{
  \begin{minipage}[t]{0.49\textwidth}
    \only<1>{\includegraphics[width=0.7\columnwidth]{figures/spn-eval.pdf}}
    \only<2-8>{\includegraphics[width=0.65\columnwidth]{figures/spn-eval.pdf}}
  \end{minipage}
  \begin{minipage}[t]{0.49\textwidth}
    \only<2>{\includegraphics[width=0.7\columnwidth]{figures/spn-eval-ev-1.pdf}}
    \only<3>{\includegraphics[width=0.7\columnwidth]{figures/spn-eval-ev-2.pdf}}
    \only<4>{\includegraphics[width=0.7\columnwidth]{figures/spn-eval-ev-3.pdf}}
    \only<5>{\includegraphics[width=0.7\columnwidth]{figures/spn-eval-ev-4.pdf}}
    \only<6>{\includegraphics[width=0.7\columnwidth]{figures/spn-eval-ev-5.pdf}}
    \only<7>{\includegraphics[width=0.7\columnwidth]{figures/spn-eval-ev-6.pdf}}
    \only<8>{\includegraphics[width=0.7\columnwidth]{figures/spn-eval-ev-7.pdf}}
  \end{minipage}
}
\end{frame}
...
\begin{frame}[t]
  ...
  valid-SPN.\customcite{Vergari2016a}\par\bigskip
...
\begin{frame}
  ...
  \begin{equation*}
    ...
    \begin{cases}
      ...
      0& \text{otherwise}
    \end{cases}
  \end{equation*}\customcite{Vergari2016a}
\end{frame}
...
\frametitle{Learning SPNs}
  ...
  latent RV model, or as a NN

  \textbf{Structure learning:} build the network from data by assigning
  scores to tentative structures or by exploiting constraints

  How to learn a ``complete'' SPN:
  \begin{itemize}
  \item handcrafted structure, then parameter learning\customcite{Poon2011}\customcite{Gens2012}
  \item random structures, then parameter learning\customcite{Rashwan2016}
  \item structure learning, then parameter learning (fine tuning)\customcite{Zhao2016a}
  \item learn both weight and structure at the same time\customcite{Gens2013}\customcite{Rooshenas2014}\customcite{Vergari2015}\customcite{Adel2015}\dots  
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  hyperparameters).\par
  Freely available in the \textsf{Libra}\footnote{http://libra.cs.uoregon.edu/} toolkit\customcite{Rooshenas2014}\par\bigskip
  ...
  It can cope with continuous data\customcite{Adel2015}\par\bigskip
  
  Post-learning \textbf{mergining sub-SPNs} that model ``similar'' distributions.\par
  Reducing network sizes\customcite{Rahman2016}.\par\bigskip

  Learning Relational SPNs on \textbf{first order data}
  represented in Tractable Markov Logic (TML), LearnRSPN\customcite{Nath2015}.
\end{frame}
...
\begin{frame}
  ...
  Search\customcite{Peharz2014b}.\par
  Mixing latent and deterministic mixtures as sum
  nodes (a Cutset Network is an
  SPN!)\customcite{Rahman2016}\par\bigskip
  
  \textbf{Learning DAGs} structures instead of trees.\par
  Substituting sub-structures with more complex ones by cloning mixtures\customcite{Dennis2015}\par\bigskip
  
  \textbf{Template learning} for sequence
  models.
  Stochastic local search over well defined constrained structures.
  Dynamic SPNs\customcite{Melibari2016c}\emph{$\rightarrow$ PGM'16!}\par\bigskip
\end{frame}
...
\begin{frame}
  \frametitle{``Hard'' gradients   \customcite{Poon2011}\customcite{Gens2012}}
...
\begin{frame}
  \frametitle{Hard/Soft Parameter Updating \customcite{Gens2012}}
  \begin{table}
    \centering
    \begin{tabular}{l c}
      & $\Delta w_{pc}$\\
...
\begin{frame}
  ...
  tree SPNs\customcite{Rashwan2016}\par\bigskip

  \textbf{Collapse Variational Inference} (CVB-SPN) to optimize a
  logarithmic lower bound (better than ELBO) efficiently (linear in $|S|$)\customcite{Zhao2016a}.
...
\begin{frame}
  ...
  \framesubtitle{From deep neural networks\customcite{Bengio2012}}
...
\begin{frame}
  \frametitle{Extracting Embeddings}
  \framesubtitle{Exploiting SPNs as feature extractors\customcite{Vergari2016a}  }
