...
\begin{frame}
  \frametitle{Outline}
  \bigskip
  $\rightarrow$ \textcolor{lacamdarklilac5}{\Large\textbf{Representation}}\par
  \bigskip
  $\rightarrow$ \textcolor{lacamdarklilac5}{\Large\textbf{Inference}}\par
  \bigskip
  $\rightarrow$ \textcolor{lacamdarklilac5}{\Large\textbf{Interpretation}}\par
  \bigskip
  $\rightarrow$ \textcolor{lacamdarklilac5}{\Large\textbf{Learning}}\par
  \bigskip
  $\rightarrow$ \textcolor{lacamdarklilac5}{\Large\textbf{Applications}}\par
  \bigskip
  $\rightarrow$ \textcolor{lacamdarklilac5}{\Large\textbf{Representation Learning}}\par
  \bigskip
  $\rightarrow$ \textcolor{lacamdarklilac5}{\Large\textbf{\emph{References}}}\par
\end{frame}
...
machine for the represented distribution at the same time\par\bigskip

Simple and effective algorithms to learn them\par\bigskip

Successfully employed in several applications\par\bigskip
\end{frame}
...
\begin{itemize}
  ...
  \item Autoregressive Neural Networks
  \item Sum-Product Networks  
  \item \ldots
\end{itemize}\bigskip

Once a density estimator is learned, one uses it to answer
\emph{queries}, i.e. to do \emph{\textbf{\color{violet}inference}}
...
\begin{frame}
  ...
  $$\mathsf{sc}(n)\subseteq\mathbf{X}$$\\[-25pt]
  \only<2>{\begin{itemize}
    \item the scope of a leaf node $n$ is defined as the set of RVs over which
      $\phi_{n}$ is defined\par
      E.g. $\mathsf{sc}(\colorbox{orange}{\textcolor{white}{n}})=\{X_{6}\}$
    \end{itemize}}
  \only<3>{\begin{itemize}
    \item the scope of a leaf node $n$ is the set of RVs over which
      $\phi_{n}$ is defined\par
    \item the scope of an inner node $n$ is defined as
      $\mathsf{sc}(n)=\bigcup_{c\in\mathsf{ch}(n)}\mathsf{sc}(c)$\par
      E.g. $\mathsf{sc}(\colorbox{orange}{\textcolor{white}{n}})=\{X_{5},X_{6}\}$
    \end{itemize}}
  \only<4>{\begin{itemize}
    ...
    \item the scope of an inner node $n$ is defined as
      $\mathsf{sc}(n)=\bigcup_{c\in\mathsf{ch}(n)}\mathsf{sc}(c)$\par
      E.g. $\mathsf{sc}(\colorbox{orange}{\textcolor{white}{n}})=\{X_{3},
      X_{4},X_{5},X_{6}\}$
    \end{itemize}}
  \only<5>{\begin{itemize}
    ...
    \item the scope of an inner node $n$ is defined as
      $\mathsf{sc}(n)=\bigcup_{c\in\mathsf{ch}(n)}\mathsf{sc}(c)$\par
      E.g.\par$\mathsf{sc}(\colorbox{orange}{\textcolor{white}{n}})=\{X_{1},X_{2},X_{3},
      X_{4},X_{5},X_{6}\}$
    \end{itemize}}
  \only<6>{\begin{itemize}
    ...
    \item  the scope of $S$ is the scope of its root, i.e. $\mathbf{X}$\par
      E.g.\par $\mathsf{sc}(S)= \mathsf{sc}(\colorbox{orange}{\textcolor{white}{n}})=\{X_{1},X_{2},X_{3},
      X_{4},X_{5},X_{6}\}$
    \end{itemize}}
\end{minipage}\hfill\begin{minipage}{0.4\textwidth}
  ...
  \only<2>{\includegraphics[width=0.75\columnwidth]{figures/spn-scope-1.pdf}}
  \only<3>{\includegraphics[width=0.75\columnwidth]{figures/spn-scope-2.pdf}}
  \only<4>{\includegraphics[width=0.75\columnwidth]{figures/spn-scope-3.pdf}}
  \only<5>{\includegraphics[width=0.75\columnwidth]{figures/spn-scope-4.pdf}}
  \only<6>{\includegraphics[width=0.75\columnwidth]{figures/spn-scope-5.pdf}}
\end{center}
\end{minipage}
\end{frame}
...
\end{minipage}\hfill\begin{minipage}[t]{0.58\linewidth}
  \vspace{-80pt}
  To compute $p(\mathbf{X}=\mathbf{x})$, evaluate $S$ in a bottom-up (feedforward)
  fashion.\par\bigskip
...
\begin{frame}
  ...
  \begin{minipage}{0.4\linewidth}
    ...
    \begin{center}
      \only<1>{\includegraphics[width=0.75\columnwidth]{figures/spn-eval.pdf}}
      \only<2>{\includegraphics[width=0.75\columnwidth]{figures/spn-eval-marg-1-alt}}
      \only<3>{\includegraphics[width=0.75\columnwidth]{figures/spn-eval-marg-2}}
    \end{center}
  \end{minipage}\hfill\begin{minipage}[t]{0.58\linewidth}
    \vspace{-80pt}
    To compute a marginal query like $p(\mathbf{Q=q}),
    \mathbf{Q}\subset\mathbf{X}$ evaluate $S$ as before (feedforward)\par
    \only<2>{but evaluate a leaf $n$ as:
    $$S_{n}(\mathbf{q}) = \begin{cases}
      p(\mathsf{sc}(n) =\mathbf{q}_{|\mathsf{sc}(n)}) & \text{if $\mathsf{sc}(n)\subseteq\mathbf{Q}$} \\
      \colorbox{purple}{\textcolor{white}{1.0}} &
      \text{otherwise}\end{cases}$$}
  \only<3>{but evaluate a leaf $n$ as:
    $$S_{n}(\mathbf{q}) = \begin{cases}
      p(\mathsf{sc}(n) =\mathbf{q}_{|\mathsf{sc}(n)}) & \text{if $\mathsf{sc}(n)\subseteq\mathbf{Q}$} \\
      1.0 & \text{otherwise}\end{cases}$$}
  \only<3>{then propagate as before
    \begin{itemize}
    \item each sub-network shall output 1 as the probability of
      marginalizing over all the RVs out of its scope
    \end{itemize}
\par
  Conditionals are tractable as well:\par
  $p(\mathbf{Q}|\mathbf{E}) = p(\mathbf{Q}, \mathbf{E})/p(\mathbf{E})$}
  \end{minipage}
\end{frame}

\begin{frame}
  \frametitle{MPE inference\customcite{Peharz2016}}

  \begin{minipage}{0.4\linewidth}
    \vspace{10pt}
    \begin{center}
      \only<1>{\includegraphics[width=0.75\columnwidth]{figures/spn-eval}}
      \only<2>{\includegraphics[width=0.75\columnwidth]{figures/spn-eval-mpe-1.pdf}}
      \only<3>{\includegraphics[width=0.75\columnwidth]{figures/spn-eval-mpe-2.pdf}}
      \only<4>{\includegraphics[width=0.75\columnwidth]{figures/spn-eval-mpe-3.pdf}}
      \only<5>{\includegraphics[width=0.75\columnwidth]{figures/spn-eval-mpe-4-alt.pdf}}
      \only<6>{\includegraphics[width=0.75\columnwidth]{figures/spn-eval-mpe-5-alt.pdf}}
    \end{center}
  \end{minipage}\hfill\begin{minipage}[t]{0.60\linewidth}
    \vspace{-80pt}
    \only<1>{An approximation of MPE inference can be answered in linear time as
      well
      $$\mathbf{q}^{*}=\argmax_{\mathbf{q}\sim \mathbf{Q}}p(\mathbf{E},
      \mathbf{q})$$
      for some RVs $\mathbf{E}, \mathbf{Q} \subset\mathbf{X},
      \mathbf{E}\cap\mathbf{Q}=\emptyset,$\par
      $\mathbf{E}\cup \mathbf{Q}=\mathbf{X}$ }
    \only<2-6>{An approximation of MPE inference can be answered in linear time as
      well.\par}
      \only<2-4>{eg: {\scriptsize $\mathbf{Q}=\{X_{3},X_{4},X_{5}\},\mathbf{E}=\{X_{1},X_{2},X_{6}\}$}}
      \only<5>{eg:
        {\scriptsize$\mathbf{Q}=\{$$X_{3},X_{4},X_{5}$$\},\mathbf{E}=\{$\colorbox{black}{\textcolor{white}{$X_{1},X_{2}\}$}}$,X_{6}$}}
      \only<6>{eg:
      {\scriptsize$\mathbf{Q}=\{$\colorbox{purple}{\textcolor{white}{$X_{3},X_{4},X_{5}$}}$\},\mathbf{E}=\{$\colorbox{black}{\textcolor{white}{$X_{1},X_{2},X_{6}\}$}}}}
      
    \only<2>{\begin{itemize}
    \item build a \textbf{\color{violet}\emph{Max-Product Network}} $M$ substituting each $n\in \mathbf S_{\oplus}$ for a \textbf{max} node computing
      $$\max_{c\in\mathsf{ch}(n)}w_{nc}M_{n}$$
    \end{itemize}}
    \only<3-6>{\begin{itemize}
    \item<3-6> build a \textbf{\color{violet}\emph{Max-Product
          Network}} $M$
    \item<3-6> evaluate $M$ bottom-up after setting all leaves $n$, $\mathsf{sc}(n)\subseteq
      \mathbf{Q}$ to 1
    \item<4-6> a top-down traversal traces back the MPE
      assignment for each RV in $\mathbf{Q}$, following:
      \begin{itemize}
      \item<4-6> only the max output
        child branch of a max node
      \item<5-6> all child branches of product nodes
      \end{itemize}
    \item<6>{determining a {\color{red}\textbf{path}} whose leaves union forms the
        MPE assignment}
    \end{itemize}}
...
\begin{frame}
\frametitle{Partition function computation}
\begin{center}
\onslide<1-2>{
    \only<1>{\includegraphics[width=0.3\columnwidth]{figures/spn-eval-part-1.pdf}}
    \only<2>{\includegraphics[width=0.3\columnwidth]{figures/spn-eval-part-2.pdf}}
}
\end{center}
...
\begin{frame}
  ...
  \item SPNs are not classical PGMs
    \begin{itemize}
    \item they are computational graphs, inference machines,\dots
    \end{itemize}
...
\begin{frame}[t]
  ...
  SPNs are a particular kind of \emph{\color{violet}\textbf{labelled}},
  \emph{\color{violet}\textbf{constrained}} and \emph{\color{violet}\textbf{fully probabilistic}}
  ...
  SPNs provide a \emph{\textbf{\color{violet}direct encoding}} of the input space into a deep
  architecture $\rightarrow$ \emph{visualizing representations} (back) into the \emph{input space}.
\end{frame}
...
\begin{frame}
  ...
  \begin{center}
    \begin{minipage}{0.22\linewidth}
      \includegraphics[width=1.0\columnwidth]{figures/spn-eval}
    \end{minipage}\hspace{35pt}\begin{minipage}{0.215\linewidth}
      \includegraphics[width=1.0\columnwidth]{figures/spn-eval-mpe-5-alt}      
    \end{minipage}\hspace{37pt}\begin{minipage}{0.22\linewidth}
      \includegraphics[width=1.0\columnwidth]{figures/mpe-filters-tall}      
    \end{minipage}
  \end{center}
\end{frame}
...
\begin{frame}
  ...
  \emph{$\rightarrow$ First principled top-down learner, inspired many algorithms and
  variations}\par
  \emph{$\rightarrow$ Surprisingly simple and accurate}
\end{frame}
...
\begin{frame}
  ...
  \emph{$\rightarrow$ one hyperparameter less}, $\lambda$.\par%\bigskip
  \emph{$\rightarrow$ not committing to complex structures too early}\par
  \emph{$\rightarrow$ reducing node out fan increases the depth}\par
  \emph{$\rightarrow$ same expressive power as LearnSPN structures}\par
  \emph{$\rightarrow$ statistically same (or better) accuracy, smaller
  ...
  \begin{table}
    ...
    \begin{tabular}{l r r r r r}
      ...
      & \textsf{CVB-SPN}\customcite{Zhao2016a} &
                                                 \textsf{OBMM}\customcite{Rashwan2016} & \textsf{SGD}\footnotemark[21] & \textsf{EM}\footnotemark[21] & \textsf{SEG}\footnotemark[21] \\
      \midrule
  ...
  \begin{table}
    ...
    \begin{tabular}{l r r r r r r r}
      ...
      & \textsf{LearnSPN}\customcite{Gens2013} &
                                                 \textsf{LearnSPN-b}\customcite{Vergari2015} & \textsf{CVB-SPN} & \textsf{OBMM}& \textsf{SGD} & \textsf{EM} & \textsf{SEG} \\
      \midrule
...
\begin{frame}
  ...
  E.g. deriving EM update rules for leaf distributions~\parencites{Peharz2015a,Desana2016}\par\bigskip
...
\begin{frame}
  ...
  Image reconstruction and \emph{inpainting}: fill the missing pixels of
  test samples by the means of efficient MPE inference.\par
  Fixed (taking spatial autocorrelation into account) or
  learned structures.\par
  \par\bigskip
  \begin{minipage}{0.5\linewidth}
    \begin{center}
      \includegraphics[width=1.0\columnwidth]{figures/mpe-faces}
    \end{center}
  \end{minipage}\hfill\begin{minipage}[t]{0.45\linewidth}
    \vspace{-40pt}
    Reconstructing some simmetries (eyes, but not beards, glasses).\par\bigskip
    Testing different approximations for MPE inference~\parencite{Peharz2015b}.
...
\begin{frame}
  \frametitle{Applications II: activity recognition}
  Videos represented as regular grids of points in space and time,
  described by Bag-of-Words (BoW).\par
  An SPN structure models a hierarchy of BoW products.\par
  Inference for activity recognition \emph{and localization}.
  \begin{center}
    \includegraphics[width=0.3\columnwidth]{figures/amer-fig-1}\hfill
    \includegraphics[width=0.2\columnwidth]{figures/amer-fig-2}\hfill
    \includegraphics[width=0.32\columnwidth]{figures/amer-fig-3}
  \end{center}
  {\color{violet}\textbf{Exploiting part based decomposability}} along pixels \emph{and time} (frames).
  \customcitenomark{Amer2012}
  \customcitenomark{Amer2015}
\end{frame}

\begin{frame}
  \frametitle{Applications III: speech}
...
\begin{frame}
  \frametitle{Applications IV: language modeling}
  Fixed structure SPN encoding the conditional probability
  {\footnotesize $p(w_{i}|w_{i-1},\dots,w_{i-n})$} as an $n$-th order language model.\par\bigskip
  \begin{minipage}{0.4\linewidth}
    \begin{center}
      \includegraphics[width=1.0\columnwidth]{figures/cheng-fig}
    \end{center}
  \end{minipage}\hfill\begin{minipage}[t]{0.55\linewidth}
    \vspace{-50pt}
    One-hot encoding of word vocabulary.\par
    Windowed representation of size\par\bigskip
    First \textbf{\color{violet}embedding layer} with size $D$,\par
    sharing word weights across different\par
    mixtures (position invariance).
    
  \end{minipage}\\[10pt]
  State-of-the-art \emph{perplexity} on PennTreeBank even for low
  orders ($n=4$).
  
  \customcitenomark{Cheng2014}
\end{frame}
...
\begin{frame}
  ...
  \item power law distribution of nodes by scopes
    ...
    $\rightarrow$ Which filtering criterion to employ?\par
    $\rightarrow$ Which interpretation for the extracted features?
...
\begin{frame}
  ...
  \begin{table}[!t]
    ...
    \label{tab:model-accs}
  \end{table}\bigskip
  \emph{$\rightarrow$ comparable or better accuracies than (intractable) RBM embeddings}
  \customcitenomark{Vergari2016a}
\end{frame}
...
\begin{frame}
  ...
  \begin{table}[!t]
    \centering
    \footnotesize
    \caption[datasets]{Test set accuracies for embeddings
      filtered by node
      type and by \textsf{S}mall
  ...
  \emph{$\rightarrow$ sum nodes only are good compressors for larger
    models}\par
  \emph{$\rightarrow$ mid size scope length embeddings provide best
...
\begin{frame}
  ...
  \begin{table}[!t]
    ...
    \footnotesize
    \caption[datasets]{Test set accuracies for embeddings
      by aggregating node outputs
      with the same scope, with and without leaves.
      Bold values denote significantly better scores than all the others
      for each dataset.
      $\blacktriangle$ indicates a better score than an RBM embedding
      with greater or equal size. $\triangledown$ indicates worse
  ...
  \end{table}\vspace{-1pt}
  \emph{$\rightarrow$ shorter but still accurate embeddings}\par
  \emph{$\rightarrow$ leaves greatly contribute for smaller models}
  \customcitenomark{Vergari2016a}
\end{frame}
...
\begin{frame}
  \frametitle{Random Marginal Queries}
   \begin{center}
    \includegraphics[width=0.37\linewidth]{figures/lines-rectangles}\hspace{20pt}\includegraphics[width=0.37\linewidth]{figures/lines-convex}\\
    \includegraphics[width=0.37\linewidth]{figures/lines-ocr_letters}\hspace{20pt}\includegraphics[width=0.37\linewidth]{figures/lines-bmnist}
  \end{center}
  \emph{$\rightarrow$ structure learning is meaningful!}\par
  \emph{$\rightarrow$ an SPN representation power goes beyond single nodes}
  \customcitenomark{Vergari2016a}
\end{frame}
...
\begin{frame}
  ...
  \url{https://github.com/arranger1044/awesome-spn}\par\bigskip

  \only<2>{
    \textbf{\color{lacamdarklilac5}Thanks to the research teams
      working on SPNs!}\par\bigskip
    \textbf{\color{violet}University of Washington}: Domingos, Poon, Gens\par
    \textbf{\color{violet}University of Graz}: Peharz, Pernkopf, Tschiatschek\par
    \textbf{\color{violet}University of Waterloo}: Poupart, Zhao, Adel, Melibari, Rashwan, Jaini\par
    \textbf{\color{violet}University of Oregon}: Lowd, Rooshenas\par
    \textbf{\color{violet}University of Bari}: DiMauro, Vergari\par
    \textbf{\color{violet}Oregon State University}: Todorovic, Amer\par
    \textbf{\color{violet}University of Texas at Dallas}: Gogate,
    Rahman\par
    \dots
  }
\end{frame}
...
\begin{frame}
  ...
  \begin{center}
    \begin{minipage}{0.25\linewidth}
      \includegraphics[width=1.0\columnwidth]{figures/grid-2}
    \end{minipage}\hspace{35pt}\begin{minipage}{0.215\linewidth}
      \vspace{12pt}
      \includegraphics[width=1.0\columnwidth]{figures/mpe-faces-tall}      
    \end{minipage}\hspace{35pt}\begin{minipage}{0.22\linewidth}
      \includegraphics[width=1.0\columnwidth]{figures/spn-emb-eval}      
    \end{minipage}
     \end{center}
\end{frame}
